...
これは TypeScript で構築された Astro ベースのブログで、ブログ記事と日記エントリーの両方を特徴とします。コンテンツはmicroCMSで管理されています。サイトは以下をサポートします：
- **microCMS統合**：blog、diary、emonicleの3つのコンテンツタイプ
- **プレビュー機能**：microCMS管理画面からのリアルタイムプレビュー（Preact + SWR）
...
プレビュー機能により、microCMS管理画面から記事の下書きをリアルタイムで確認できます。

### プレビューページ（`/preview`）
- **URL**: `/preview?contentId={CONTENT_ID}&draftKey={DRAFT_KEY}&type={blog|diary|emonicle}`
- **技術スタック**: Preact + SWR（3秒ごとに自動更新）
- **機能**:
  - プレビューバナー表示
  - リアルタイム更新（3秒ごと）
  - エラーハンドリング
...
### microCMS管理画面での設定
各コンテンツタイプのプレビューURL:
- **Blog**: `https://www.kechiiiiin.com/preview?contentId={CONTENT_ID}&draftKey={DRAFT_KEY}&type=blog`
- **Diary**: `https://www.kechiiiiin.com/preview?contentId={CONTENT_ID}&draftKey={DRAFT_KEY}&type=diary`
- **Emonicle**: `https://www.kechiiiiin.com/preview?contentId={CONTENT_ID}&draftKey={DRAFT_KEY}&type=emonicle`

### 実装詳細
- **コンポーネント**: `src/components/PreviewContainer.tsx`（Preact）
- **ページ**: `src/pages/preview.astro`
- **microCMSクライアント**: `src/lib/microcms.ts`
- **環境変数**: `PUBLIC_MICROCMS_SERVICE_DOMAIN`, `PUBLIC_MICROCMS_API_KEY`
- **Preact統合**: `astro.config.mjs`に`compat: true`を設定（React互換モード）
